...
4. **Improve the Pangram label's design** (Costa, 2026-09-06): the standalone pill under the meta line was the first cut; the second is the badge folded into the meta line, copied in shape from Pangram's own extension (Costa supplied its X and LinkedIn screenshots as the reference). **Still not settled** — he called it "still needs some work" on 2026-09-06. Open: the badge sits last in the meta row, after the tags, while Pangram puts its own early, right after the name; and on the four `Mixed` posts the one word alone is harsher than `70% human` would be, with the number only in the `title`. Everything it draws is `.meta a.verdict` in `src/styles/global.css` and the `<a class="verdict">` in the meta block of `src/pages/[slug].astro`; `verdictTitle()` in `src/lib/pangram.ts` gives the tooltip.
