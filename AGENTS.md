...
src/lib/pangram.ts      verdictFor(entry): the post's Pangram verdict from src/data/pangram.json, only while its prose hash still matches; plus the label text and tooltip
...
- **Every post carries Pangram's verdict** (added 2026-09-06): a `.pill` link under the meta line (`<p class="verdict">` in `src/pages/[slug].astro`) reading Pangram's own headline lowercased plus the human percentage (`✍️ human written · 100% human`), opening Pangram's public analysis page for that post in a new tab, with the three fractions and the check date in `title`; the Markdown twin carries the same as a meta line. A `Human` verdict gets the accent border; `Mixed` (🤝) and `AI` (🤖) render the same pill with whatever Pangram said, since the label shows the verdict as-is (Costa's decision, 2026-09-06). The data is `src/data/pangram.json`, keyed by slug and written only by `scripts/pangram.mjs`; `src/lib/pangram.ts` renders an entry only while its `hash` matches the sha256 of the post's current prose (`proseOf()` in `src/lib/prose.mjs`), otherwise it warns `[pangram] no verdict for <slug> (missing|stale)` and leaves the label out, so a build never fails on it. Pages, things and the lab have no verdict. See the Pangram section.
...
4. **Improve the Pangram label's design** (Costa, 2026-09-06): the `.pill` under the meta line with the verdict text and the human percentage is the first cut, committed to get it live. Costa wants to revisit the look when he is back: pill or plain line, emoji or none, the wording, whether the percentage shows, and its placement against the meta line and the "featured in" line. Everything it draws is in `.post-head .verdict` in `src/styles/global.css` and the `<p class="verdict">` block in `src/pages/[slug].astro`; `verdictLabel()`/`verdictTitle()` in `src/lib/pangram.ts` give the text.
