...
src/components/         PostList, PostCard, TagChips, Popular (most beers), ThemeToggle, Search (Pagefind), ShareBar, Claps, Projects, Elsewhere, Things + ThingsPage (the feed)
...
- **Most beers** (`src/components/Popular.astro`, between the tag chips and the feed on `/`, and `## Most beers` in `/index.md`): the five posts with the most beers, counted at build time from `GET /top?limit=100` on the claps API (`src/lib/claps.ts`; endpoint added to `CostaFot/claps-api` on 2026-09-04). Keys are `https://www.costafotiadis.com/<slug>` with no trailing slash, the same key `Claps.astro` posts to; non-post keys (the homepage, retired URLs) are ignored. The counts are as of the last deploy, not live. When the API is down or the endpoint is missing the build warns `[claps] no beer counts` and leaves the block out, so a build never fails on it. `CLAPS_API=http://localhost:4567` points a local build at a mock (a few lines of node serving `/top` from the `claps.json` backup in `CostaFot/claps` will do).
...
2. `npm run build` also prints no `[claps]` warning, and `dist/index.html` has a `section.popular` with five rows (`grep -c 'class="popular"' dist/index.html`) and `dist/index.md` a `## Most beers` list; if the warning shows, the claps API was unreachable and the block was skipped, which is fine offline.
