...
- **Every post carries a Pangram verdict since 2026-09-06** (see the Pangram section): all 32 posts were checked that day with `pangram-4` (18,802 prose words, about $9.40 of prepaid credit). 26 came back `Human Written` at 100 %. Six did not, and their labels say so: building-things `AI Generated` (0 % human), mogged-into-building-a-chrome-extension `AI Detected` (45 %), it-looks-like-youre-trying-to-build-an-extension-for-command-palette `AI Detected` (56 %), using-a-github-file-as-a-database-cause-why-not `AI Detected` (70 %), at-the-mountains-of-madness-rewriting-a-100-line-powershell-script-as-a-kmp-desktop-app `AI Detected` (89 %), remote-compose-looks-promising `Mostly Human Written` (94.5 %, badge still `Human`). What to do about those is Costa's call; the label shows the verdict as-is by his decision. The key is in `.env` on this machine only; Costa pasted it into a chat on 2026-09-06, so rotating it in Pangram's dashboard is worth doing (only `.env` needs updating).
...
5. **Go through the six posts that did not come back 100 % human, one by one** (Costa, 2026-09-06). Not a design task: the verdicts are shown as-is and what to do about each post is his call, taken post by post. In order of how far off they are: `building-things` (0 % human, `AI` badge, "AI Generated"), `mogged-into-building-a-chrome-extension` (45 %, `Mixed`), `it-looks-like-youre-trying-to-build-an-extension-for-command-palette` (56 %, `Mixed`), `using-a-github-file-as-a-database-cause-why-not` (70 %, `Mixed`), `at-the-mountains-of-madness-rewriting-a-100-line-powershell-script-as-a-kmp-desktop-app` (89 %, `Mixed`), `remote-compose-looks-promising` (94.5 %, still `Human`, headline "Mostly Human Written"). All six are 0 % AI-assisted, so Pangram is calling whole stretches AI rather than finding edited-over prose; the analysis page for each highlights which stretches. Rewriting one in Costa's voice goes through the `costa-writing-style` skill, and **every prose edit needs `node --env-file=.env scripts/pangram.mjs` before the commit** (a few cents a post, a new analysis link each time). Never tweak prose only to chase a better verdict — fix the writing, then re-check.
