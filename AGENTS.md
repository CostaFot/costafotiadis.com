...
- **Every post carries a Pangram verdict since 2026-09-06** (see the Pangram section): all 32 posts were checked that day with `pangram-4` (18,802 prose words, about $9.40 of prepaid credit). 26 came back `Human Written` at 100 %. Six did not. **Three of those were rewritten later the same day** and re-checked, and **the command-palette and mogged posts were rewritten by Costa the same day** (see phase 5), leaving the standing verdicts: building-things `AI Generated` (0 % human), it-looks-like-youre-trying-to-build-an-extension-for-command-palette `Mostly Human Written` (93.4 %, badge `Human`), mogged-into-building-a-chrome-extension `Human Written` (100 %), using-a-github-file-as-a-database-cause-why-not `AI Detected` (83.7 %), at-the-mountains-of-madness-rewriting-a-100-line-powershell-script-as-a-kmp-desktop-app `Mostly Human Written` (92.4 %, badge `Human`), remote-compose-looks-promising `Mostly Human Written` (94.5 %, badge `Human`). What to do about the rest is Costa's call; the label shows the verdict as-is by his decision. The key is in `.env` on this machine only; Costa pasted it into a chat on 2026-09-06, so rotating it in Pangram's dashboard is worth doing (only `.env` needs updating).
...
4. `npm start` (with `media/` holding the videos, or `MEDIA_DIR=…`), then curl: `/` 200, `/<slug>/` 200, `/elsewhere/` 200 and `/elsewhere.md` with the list under `## Newsletters`, `/<slug>` 301 to the slash form, `/tag/android/` 200, `/stats/` 200, `/adb-extension-stats/` 301 to `/stats/`, `/rss.xml` 200, `/content/images/2026/07/image.png` 200, `/things-feed/` 301 to `/things/`, `/nope/` 404. The lab: `/lab/` 200, `/lab` 301, `/lab/drive/` 200 (also for a plain `curl`, as HTML, since there is no twin), `/lab/nope/` 404. The feed: `/things/` 200, `/things/tag/life/` 200, `/images/things/20260902_171522.jpg` 200, `curl -sI -H 'Range: bytes=0-99' /media/20260827_234011.mp4` 206 with `Content-Range`. Then the Markdown side: `/<slug>.md` (with the `Pangram analysis` line under the meta), `/things.md`, `/index.md` and `/llms.txt` 200 as `text/markdown`/`text/plain`; a plain `curl /<slug>/` returns Markdown with `cache-control: no-store`, while `curl -A Mozilla /<slug>/` and `curl -H 'Accept: text/html' /<slug>/` return HTML with the normal cache header.
...
5. **The six posts that did not come back 100 % human** (Costa, 2026-09-06). Not a design task: the verdicts are shown as-is and what to do about each post is his call, taken post by post. Rewriting one in Costa's voice goes through the `costa-writing-style` skill, and **every prose edit needs `node --env-file=.env scripts/pangram.mjs` before the commit** (a few cents a post, a new analysis link each time). Never tweak prose only to chase a better verdict — fix the writing, then re-check.
   ...
   **Done on 2026-09-06** ($3.11 of credit, four posts rewritten and re-checked):
   ...
   - `using-a-github-file-as-a-database-cause-why-not` 70 % → **83.7 %**, still `Mixed`. Three sentences in the race-condition and frontend blocks.
   ...
   **Left, exactly:**
   - `building-things` — **0 % human, `AI` badge, untouched.** Not just a writing problem: its 211 words describe the Telegram bot and markdown repo retired on 2026-09-03, so it is obsolete on its own merits. Rewrite it as how the feed works now, or retire it, before worrying about the verdict.
