...
- Never edit `src/data/pangram.json` by hand and never delete an entry to hide it; the script prunes entries whose slug no longer exists.
...
5. **The six posts that did not come back 100 % human** (Costa, 2026-09-06). Not a design task: the verdicts are shown as-is and what to do about each post is his call, taken post by post. Rewriting one in Costa's voice goes through the `costa-writing-style` skill, and **every prose edit needs `node --env-file=.env scripts/pangram.mjs` before the commit** (a few cents a post, a new analysis link each time). Never tweak prose only to chase a better verdict — fix the writing, then re-check.
   ...
   **Done on 2026-09-06** ($1.28 of credit, three posts rewritten and re-checked):
   ...
   **Left, exactly:**
   ...
   - `it-looks-like-youre-trying-to-build-an-extension-for-command-palette` — **56 %, `Mixed`. Costa is doing this one himself.** A rewrite was tried on 2026-09-06 and **reverted**: tightening the 392-word MSIX/code-signing section took it to 53 % and pushed that window's confidence from Medium to High. Compression reads as *more* AI there, not less. Do not touch the opener ("hit ↑ seventeen times, squint") — it is flagged only at Medium and the fake-precise number is his signature.
   ...
   - **Reverting a prose edit restores the old verdict for free.** The original prose hashes back to its original value, so the previous entry can be restored from git (`git show HEAD:src/data/pangram.json`) instead of re-checking. That is how the command-palette revert cost nothing.
