...
src/lib/claps.ts        beer counts from the claps API's /top at build time; popularPosts() picks the top five for the homepage and /index.md
...
src/data/medium-claps.json  snapshot of Medium claps per story (2026-09-04), with the slug here or null for the six not migrated; see Next phases
...
2. `npm run build` also prints no `[claps]` warning, `dist/tag/popular/index.html` has five cards with `beers` in their meta, the chip row on `dist/index.html` has `/tag/popular/`, and `dist/index.md` a `## Most beers` list; if the warning shows, the claps API was unreachable and the chip was skipped, which is fine offline.
...
7. Every path in the live sitemaps (`sitemap-posts.xml`, `sitemap-pages.xml` on www.costafotiadis.com) must exist as `dist/<path>/index.html` until Ghost is gone, or be in the `REDIRECTS` table in `server.js`. Retired so far: `/adb-extension-stats/` (2026-09-02) redirects to `/stats/`; `RETIRED_SLUGS` in `scripts/convert.js` keeps the converter from regenerating it.
...
4. **TODO: make the beer ranking live, not build-time.** `/tag/popular/` and the `🍺 popular` chip are a snapshot of the claps API taken during `npm run build`, so counts lag until the next deploy. Options, undecided: a small script on that page that fetches `/top` in the browser and reorders or re-counts the cards (the page still renders without JS, from the build snapshot), or a scheduled rebuild. Either way keep the static page as the fallback.
5. **TODO: migrate the remaining Medium stories.** Six of the 25 Medium stories were never brought over: Retrofit review (2019-02-18, 76 unique clappers), Retrofit re-review: the clappening (2019-02-24), Glide review (2019-02-19), Kotlin (over) Flow review (2020-10-18), Using Android RecyclerView in 2019 (2019-02-14) and Android RxJava in 5 minutes (2019-02-08). They are the `slug: null` rows in `src/data/medium-claps.json`, a snapshot of Medium's per-story counts taken 2026-09-04 (`unique_clappers` is Medium's `voterCount`, `claps` its `clapCount`, read from Medium's GraphQL at `/_/graphql` while logged in; the old `?format=json` endpoint is gone). The same file maps the other 19 stories to their slugs here, so the counts can seed the beer counts once that is decided (fold into the claps API totals, or show as a separate "from Medium" figure). Medium is not the source any more; a migrated story needs its slug filled in there. **The 19 mapped counts were imported into the claps API on 2026-09-04** with `scripts/medium-claps.sql` (one upsert per post, adding the unique clappers to the existing count; do not run it again). `scripts/medium-claps-before.json` is `/top` as it was just before, for undoing it. psql against the Postgres service's `DATABASE_PUBLIC_URL` (Railway's TCP proxy) timed out on most connection attempts that day; a retry loop got through.
6. **Apex on Railway (optional)**: the bare `costafotiadis.com` still goes through the Caddy redirector at `178.128.137.126`. If that box is retired, add the apex as a custom domain on the `website` service and use Wix's A/ALIAS option, since Wix cannot CNAME an apex.
