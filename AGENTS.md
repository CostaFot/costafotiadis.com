...
- **Every post carries a Pangram verdict since 2026-09-06** (see the Pangram section): all 32 posts were checked that day with `pangram-4` (18,802 prose words, about $9.40 of prepaid credit). 26 came back `Human Written` at 100 %. Six did not, and their labels say so: building-things `AI Generated` (0 % human), mogged-into-building-a-chrome-extension `AI Detected` (45 %), it-looks-like-youre-trying-to-build-an-extension-for-command-palette `AI Detected` (56 %), using-a-github-file-as-a-database-cause-why-not `AI Detected` (70 %), at-the-mountains-of-madness-rewriting-a-100-line-powershell-script-as-a-kmp-desktop-app `AI Detected` (89 %), remote-compose-looks-promising `Mostly Human Written` (95 %). What to do about those is Costa's call; the label shows the verdict as-is by his decision. The key is in `.env` on this machine only; Costa pasted it into a chat on 2026-09-06, so rotating it in Pangram's dashboard is worth doing (only `.env` needs updating).
...
- **Every post carries Pangram's verdict** (added 2026-09-06, reshaped the same day): the last item in a post's meta line is an `<a class="verdict">` badge built to look like the one Pangram's own extension draws on X and LinkedIn — a coloured dot, the verdict in one word, and a `›` chevron, in a tinted box of the same colour. The word is `prediction_short` verbatim (`Human`, `Mixed`, `AI`), so it says what the extension would say about the same text; the link opens Pangram's public analysis page in a new tab, with the three fractions, the model and the check date in `title`. Colours are the `--ok`/`--warn`/`--bad` tokens (green/amber/red, one value per theme); the background and border tints come off `color-mix` of the same colour, so a state is one value. No percentage and no emoji on the badge — the Markdown twin keeps the fuller `human written · 100% human · [Pangram analysis](…)` line instead, having no colour to carry the signal. `Mixed` and `AI` render the same badge with whatever Pangram said, since the label shows the verdict as-is (Costa's decision, 2026-09-06). The data is `src/data/pangram.json`, keyed by slug and written only by `scripts/pangram.mjs`; `src/lib/pangram.ts` renders an entry only while its `hash` matches the sha256 of the post's current prose (`proseOf()` in `src/lib/prose.mjs`), otherwise it warns `[pangram] no verdict for <slug> (missing|stale)` and leaves the label out, so a build never fails on it. Pages, things and the lab have no verdict. See the Pangram section.
...
4. **Improve the Pangram label's design** (Costa, 2026-09-06): the standalone pill under the meta line was the first cut; the second is the badge folded into the meta line, copied in shape from Pangram's own extension (Costa supplied its X and LinkedIn screenshots as the reference). **Still not settled** — he called it "still needs some work" on 2026-09-06. Open:
   - **The click sends the reader to pangram.com and Costa does not want that** (2026-09-06, the strongest of the three). It is `target="_blank"`, so nothing navigates away, but a new tab still hands the reader to someone else's site. Ideas, none decided: a disclosure on the badge itself (`<details>` or the native popover API, both without JS, so the no-JS invariant holds) showing the fractions, model and date in place, with the analysis link inside it for whoever wants the proof; or point the badge at a page here — a `/verdicts/`-style page that explains what the labels mean and lists every post's verdict, linking out to Pangram from there, which keeps the click on the site and puts the methodology in one place. Whatever it becomes, the proof has to stay reachable: an unfalsifiable label is worth nothing, which is the whole point of doing this.
   - The badge sits last in the meta row, after the tags, while Pangram puts its own early, right after the name.
   - On the four `Mixed` posts the one word alone is harsher than `89% human` would be, with the number only in the `title`. Pangram's own page for the 89 % one shows a `Mixed` badge over an `AI Detected` headline, so the word is theirs and is the softer of the two; what the badge drops is the number, not the severity. Suggested and not yet answered: the percentage on `Mixed` only, leaving `Human` as the single word Pangram uses.

   Everything it draws is `.meta a.verdict` in `src/styles/global.css` and the `<a class="verdict">` in the meta block of `src/pages/[slug].astro`; `verdictTitle()` in `src/lib/pangram.ts` gives the tooltip.
